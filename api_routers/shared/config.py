...
async def idempotency_middleware(request: Request, call_next):
    ...
    try:
        with bootstrap.uow() as uow:
            ...
            existing = uow.session.execute(
                text("SELECT response_status, response_body, is_processing, request_hash FROM idempotency_keys WHERE idempotency_key = :key AND expires_at > :now"),
            ...
            uow.session.execute(
                text("""
                    INSERT INTO idempotency_keys (idempotency_key, endpoint, request_hash, is_processing, created_at, expires_at)
                    VALUES (:key, :endpoint, :hash, true, :now, :expires)
                    ON CONFLICT (idempotency_key) DO NOTHING
                """),
                {"key": idempotency_key, "endpoint": request.url.path, "hash": payload_hash,
                 "now": datetime.now(timezone.utc), "expires": datetime.now(timezone.utc) + timedelta(hours=24)}
            )
            uow.commit()

            # Verify the key wasn't used with a different payload
            check = uow.session.execute(
                text("SELECT request_hash FROM idempotency_keys WHERE idempotency_key = :key"),
                {"key": idempotency_key}
            ).fetchone()
            if check and check[0] and check[0] != payload_hash:
    ...
    scope = request.scope
    scope["_idempotency_body"] = body_bytes
