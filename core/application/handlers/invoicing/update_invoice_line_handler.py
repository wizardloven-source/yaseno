# core/application/handlers/invoicing/update_invoice_line_handler.py

"""
Update Invoice Line Handler - تحديث سطر موجود في الفاتورة
"""
...
class UpdateInvoiceLineHandler(BaseHandler[UpdateInvoiceLineCommand, InvoiceDTO]):
    """Handler for updating an invoice line"""
    
    def __init__(self, uow: IUnitOfWork):
        super().__init__(uow)
    
    @require_permission(Permission.MODIFY_DRAFT)
    def handle(self, command: UpdateInvoiceLineCommand, user_context: UserContext) -> InvoiceDTO:
        with self._uow:
            ...
            unit_price_money = Money(command.unit_price, invoice.currency)
            invoice.update_line(command.line_id, command.quantity, unit_price_money, command.notes)
            invoice_repo.save(invoice)
            self._commit()
            
            logger.info(f"Line {command.line_id} updated in invoice {invoice.number} by {user_context.user_id}")
