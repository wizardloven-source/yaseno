...
from core.application.purchasing.commands import RemovePurchaseLineCommand
...
class RemovePurchaseLineHandler(BaseHandler[RemovePurchaseLineCommand, PurchaseOrderDTO]):
    
    def __init__(self, uow: IUnitOfWork):
        super().__init__(uow)
    
    @require_permission(Permission.MODIFY_DRAFT)
    def handle(self, command: RemovePurchaseLineCommand, user_context: UserContext) -> PurchaseOrderDTO:
        with self._uow:
            ...
            removed = order.remove_line(command.line_id)
            if not removed:
            ...
            logger.info(f"Line {command.line_id} removed from purchase order {order.number} by {user_context.user_id}")
