...
from core.application.purchasing.commands import UpdatePurchaseLineCommand
...
class UpdatePurchaseLineHandler(BaseHandler[UpdatePurchaseLineCommand, PurchaseOrderDTO]):
    
    def __init__(self, uow: IUnitOfWork):
        super().__init__(uow)
    
    @require_permission(Permission.MODIFY_DRAFT)
    def handle(self, command: UpdatePurchaseLineCommand, user_context: UserContext) -> PurchaseOrderDTO:
        with self._uow:
            ...
            unit_price_money = Money(command.unit_price, order.currency)
            order.update_line(command.line_id, command.quantity, unit_price_money, command.notes)
            order_repo.save(order)
            self._commit()
            
            logger.info(f"Line {command.line_id} updated in purchase order {order.number} by {user_context.user_id}")
